...
#   lcls2/psana/psana/pscalib/geometry/GeometryAccess.py
...
Usage::
    ...
    from .GeometryAccess import GeometryAccess, img_from_pixel_arrays
...
class GeometryAccess:
    ...
    # NOTE (pscalib vendoring, US-006): the psf()/info_psf()/data_psf()/
    # pixel_coords_psf() methods were dropped here -- they lazily imported
    # psana.pscalib.geometry.UtilsPSF (not vendored) and are NOT on the
    # ix/iy pixel-index derivation path used by pscalib's render.
...
# NOTE (pscalib vendoring, US-006): convert_mask2d_to_ndarray() was dropped
# here -- it carried the only cross-package import
# (from psana.detector.NDArrUtils import info_ndarr, used only in a
# logger.debug) and is NOT on the ix/iy pixel-index derivation path.
